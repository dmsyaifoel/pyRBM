...
class Body:
  ...
  def move(self, position, angles=None):
    ...
    for flexure, letter in zip(self.flexures, self.letters):
      if letter == 'A':
        ...
        if self.dim == 3 and enabletorsion == True:
          flexure.orthvector_globalA = self.rotmat@flexure.orthvector_global
      ...
      if letter == 'B':
        ...
        if self.dim == 3 and enabletorsion == True:
          flexure.orthvector_globalB = self.rotmat@flexure.orthvector_global
  ...
  def line(self):
    ...
    for point in self.reference_points:
      line.append(self.rotmat@point + self.position)

    for flexure, letter in zip(self.flexures, self.letters):
      if letter == 'A':
        line.append(flexure.attachpoint_globalA)
      else:
        line.append(flexure.attachpoint_globalB)
