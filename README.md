...
Usage: place prbm.py files in a folder of choice. Then run a new .py file within that same folder with:
...
# q.add_flexure(bodynameA, attachpoint_localA, bodynameB, attachpoint_localB) adds a flexure between two bodies, for example:
...
# The attachment points are set in the bodies' local coordinate systems.
...
# Note: You can move a body then attach new flexures to make a prestressed compliant mechanism.

# q.add_force(bodyname, vector, attachpoint_local)  adds a force to a body, in Newton, for example:
